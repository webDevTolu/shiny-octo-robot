...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-blog-prod-e04c
  fly apps create remix-blog-prod-e04c-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-blog-prod-e04c
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-blog-prod-e04c-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-blog-prod-e04c-staging' does not match app name in config file 'remix-blog-prod-e04c'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-blog-prod-e04c-db
  fly postgres attach --app remix-blog-prod-e04c remix-blog-prod-e04c-db

  fly postgres create --name remix-blog-prod-e04c-staging-db
  fly postgres attach --app remix-blog-prod-e04c-staging remix-blog-prod-e04c-staging-db
  ```
